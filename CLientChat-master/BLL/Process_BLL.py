...
##############################################client
    # [Thong bao] client ket noi den server
    def MsgClientconnectServer(self,Port):
        if Port==8888:
            Server='1'
        elif Port==8889:
            Server='2'
        
        Message =bcolors.BOLD+bcolors.OKBLUE + "[ " + self.getCurrentDateTime('t') + " ]"+bcolors.WARNING+" Đang kết nối đến "+bcolors.OKGREEN+"- [ SERVER "+Server+" ]"+bcolors.ENDC+"\n"
        print Message

    # [Thong bao] client ket noi den server that bai
    def MsgClientconnectServerFail(self,Port):
        if Port==8888:
            Server='1'
        elif Port==8889:
            Server='2'
        
        Message =bcolors.BOLD+bcolors.OKBLUE + "[ " + self.getCurrentDateTime('t') + " ]"+bcolors.FAIL+" Không thể Kết nối đến "+bcolors.OKGREEN+"- [ SERVER "+Server+" ]"+bcolors.ENDC+"\n"
        print Message

    # [Thong bao] client ket noi den server thanh cong
    def MsgClientconnectServerOK(self,Port):
        if Port==8888:
            Server='1'
        elif Port==8889:
            Server='2'
        
        Message =bcolors.BOLD+bcolors.OKBLUE + "[ " + self.getCurrentDateTime('t') + " ]"+bcolors.OKBLUE+" Đã Kết nối đến "+bcolors.OKGREEN+"- [ SERVER "+Server+" ]"+bcolors.ENDC+"\n"
        print Message
class bcolors:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
